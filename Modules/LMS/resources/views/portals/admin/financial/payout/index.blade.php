            ...
            <table
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                    @foreach ($payoutRequests as $payoutRequest)
                        ...
                        @endphp
                        <tr>
                            <td class="px-3.5 py-4"> <h6 class="text-md leading-none"> <a href="#">#{{ $payoutRequest->payout_number }}</a> </h6></td>
                            <td class="px-3.5 py-4"> <h6 class="text-md leading-none"> 
                            ...
                            <td class="px-3.5 py-4">${{ $payoutRequest->amount }}</td>
                            <td class="px-3.5 py-4">{{ customDateFormate($payoutRequest->created_at, $format ='d M y h:i A') }}</td>
                            <td class="px-3.5 py-4">
                                @if( $payoutRequest->status =='approved')
                                   <span class="badge b-outline badge-success-outline">  {{  translate( $payoutRequest->status) }}</span>
                                @else 
                                <select name="status" class="select-status-change form-input form-select" data-action="{{ route('payout.status', ['id' => $payoutRequest->id ]) }}">
                                    <option selected disabled>{{ translate('Selected status Payout') }}</option>
                                    <option value="pending" {{ $payoutRequest->status =='pending' ? 'selected':'' }}>{{ translate('Pending') }}</option>
                                    <option value="approved" {{ $payoutRequest->status =='approved' ? 'selected':'' }}>{{ translate('Approved') }}</option>
                                </select>
                                @endif
                            </td>
                        </tr>
